   ...
   setup: function(){
      ...
      switch(this.options.type){
         ...
         case 'confirm' : this.d4.innerHTML = '<p>' + this.mes + '</p>';
            ...
            break;
        case 'timestamp' :
            var option = function (num, selected) {
            ...
            //year
            this.ts_y = document.createElement('select');
            this.ts_y.name = "year";
            this.ts_y.id = "form_litbox_" + this.ts_y.name;
            this.ts_y.appendChild(option(2007, false));
            this.ts_y.appendChild(option(2006, true));
            this.ts_y.appendChild(option(2005, false));
            this.d4.appendChild(this.ts_y);

            //month
            this.ts_m = document.createElement('select');
            this.ts_m.name = "month";
            this.ts_m.id = "form_litbox_" + this.ts_m.name;
            for (i = 1; i <= 12; i++) {
                ...
                this.ts_m.appendChild(option(zeropadding(i, 2), selected));
            }
            this.d4.appendChild(this.ts_m);

            //day
            this.ts_d = document.createElement('select');
            this.ts_d.name = "day";
            this.ts_d.id = "form_litbox_" + this.ts_d.name;
            for (i = 1; i <= 31; i++) {
                if (date.getDate() == i) {
                    selected = true;
                } else {
                ...
                this.ts_d.appendChild(option(zeropadding(i, 2), selected));
            }
            this.d4.appendChild(this.ts_d);

            //hour
            this.ts_h = document.createElement('select');
            this.ts_h.name = "hour";
            this.ts_h.id = "form_litbox_" + this.ts_h.name;
            for (i = 0; i <= 23; i++) {
                if (date.getHours() == i) {
                    selected = true;
                } else {
                ...
                this.ts_h.appendChild(option(zeropadding(i, 2), selected));
            }
            this.d4.appendChild(this.ts_h);

            //minute
            this.ts_i = document.createElement('select');
            this.ts_i.name = "minute";
            this.ts_i.id = "form_litbox_" + this.ts_i.name;
            for (i = 0; i <= 59; i++) {
                ...
                this.ts_i.appendChild(option(zeropadding(i, 2), selected));
            }
            this.d4.appendChild(this.ts_i);

            //second
            this.ts_s = document.createElement('select');
            this.ts_s.name = "second";
            this.ts_s.id = "form_litbox_" + this.ts_s.name;
            for (i = 0; i <= 59; i++) {
                if (date.getSeconds() == i) {
                    selected = true;
                } else {
                    selected = false;
                }
                this.ts_s.appendChild(option(zeropadding(i, 2), selected));
            }
            this.d4.appendChild(this.ts_s);

            var send_target = this.options.target;
            send = document.createElement('input');
            send.type = 'button';
            send.value = 'send';
            send.onclick = function () {
                var ts_y = document.getElementById('form_litbox_year').value;
                var ts_m = document.getElementById('form_litbox_month').value;
                var ts_d = document.getElementById('form_litbox_day').value;
                var ts_h = document.getElementById('form_litbox_hour').value;
                var ts_i = document.getElementById('form_litbox_minute').value;
                var ts_s = document.getElementById('form_litbox_second').value;
                var result = ts_y + "-" + ts_m + "-" + ts_d + " ";
                result += ts_h + ":" + ts_i + ":" + ts_s;
                var target = document.getElementById(send_target);
                target.value = result;
            }
            this.d4.appendChild(send);
